...
//    ---     Currency control component for AI Suite  ---
//    ------------------------------------------------------------
//
...
export class CurrencyInputComponent implements ControlValueAccessor {
  ...
  constructor() {
    this.keys = Object.keys(this.currencies).filter(k => !isNaN(Number(k)));
  }
  touch() {
    this.onTouched();
  }
  ...
  fetchEnumElement(uskey: string): string {
    return (this.currencies as Record<string, unknown>)[uskey] as string;
  }
